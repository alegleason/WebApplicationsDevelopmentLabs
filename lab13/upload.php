<?php
    ...
    if($_SESSION["name"] == "alegleason"){
        ...
        // Allow certain file formats, not used bc I validate that previously
        if($imageFileType != "jpg" && $imageFileType != "png" && $imageFileType != "jpeg") {
            $_SESSION["error_archivo"]= "Sorry, only JPG, JPEG, & PNG files are allowed.";
            $uploadOk = 0;
        }
        // Check if $uploadOk is set to 0 by an error
        if ($uploadOk == 0) {
            $_SESSION["error_archivo"]="Sorry, your file was not uploaded.";
